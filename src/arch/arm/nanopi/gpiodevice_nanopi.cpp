...
// -----------------------------------------------------------------------------
  DeviceNanoPi::DeviceNanoPi() : Device () {
...
// -----------------------------------------------------------------------------
  Pin::Mode
  DeviceNanoPi::mode (const Pin * pin) const {
    Pin::Mode ret;
    PioBank * b;
    int i, r, m;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  void
  DeviceNanoPi::setMode (const Pin * pin, Pin::Mode m) {
    PioBank * b;
    int i, r, v;
    int g = pin->mcuNumber();
    int f = g;
...
// -----------------------------------------------------------------------------
  Pin::Pull
  DeviceNanoPi::pull (const Pin * pin) const {
    PioBank * b;
    int i, r, v;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  void
  DeviceNanoPi::setPull (const Pin * pin, Pin::Pull p) {
    PioBank * b;
    int v = -1;
    int g = pin->mcuNumber();
    int f = g;
...
// -----------------------------------------------------------------------------
  void
  DeviceNanoPi::write (const Pin * pin, bool v) {
    PioBank * b;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  void
  DeviceNanoPi::toggle (const Pin * pin) {
    PioBank * b;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  bool
  DeviceNanoPi::read (const Pin * pin) const {
    PioBank * b;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  ...
  DeviceNanoPi::modes() const {
    return _modes;
  }


// -----------------------------------------------------------------------------
  const std::map<Pin::Mode, std::string> DeviceNanoPi::_modes = {
    {Pin::ModeInput, "in"},
    {Pin::ModeOutput, "out"},
    {Pin::ModeAlt2, "alt2"},
    {Pin::ModeAlt3, "alt3"},
...
// -----------------------------------------------------------------------------
  const std::map<int, Pin::Mode> DeviceNanoPi::_int2mode = {
    {0, Pin::ModeInput},
    {1, Pin::ModeOutput},
    {2, Pin::ModeAlt2},
    {3, Pin::ModeAlt3},
...
// -----------------------------------------------------------------------------
  const std::map<Pin::Mode, int> DeviceNanoPi::_mode2int = {
    {Pin::ModeInput, 0},
    {Pin::ModeOutput, 1},
    {Pin::ModeAlt2, 2},
    {Pin::ModeAlt3, 3},
  ...
  */
  const int DeviceNanoPi::_portSize[] = {
    22, 0, 17, 18, 16, 7, 14, 12, -1
  };

// -----------------------------------------------------------------------------
  PioBank *
  DeviceNanoPi::bank (unsigned int bkindex) const {
    PioBank * bk = nullptr;
...
// -----------------------------------------------------------------------------
  PioBank *
  DeviceNanoPi::pinBank (int * mcupin) const {
    PioBank * bk = nullptr;
...
// -----------------------------------------------------------------------------
  ...
  void
  DeviceNanoPi::debugPrintAllBanks () const {
...
// -----------------------------------------------------------------------------
  void
  DeviceNanoPi::debugPrintBank (const PioBank * b) const {
