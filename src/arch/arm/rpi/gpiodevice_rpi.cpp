...
// -----------------------------------------------------------------------------
  DeviceRaspberryPi::DeviceRaspberryPi() : Device () {
...
// -----------------------------------------------------------------------------
  DeviceRaspberryPi::~DeviceRaspberryPi() {
  }

// -----------------------------------------------------------------------------
  AccessLayer
  DeviceRaspberryPi::preferedAccessLayer() const {
    return AccessLayerAll;
  }

// -----------------------------------------------------------------------------
  const Gpio::Descriptor *
  DeviceRaspberryPi::descriptor() const {
    return _gpioDescriptor;
  }

// -----------------------------------------------------------------------------
  bool
  DeviceRaspberryPi::open() {
...
// -----------------------------------------------------------------------------
  Pin::Mode
  DeviceRaspberryPi::mode (const Pin * pin) const {
    Pin::Mode ret;
    PioBank * b;
    int i, r, m;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  void
  DeviceRaspberryPi::setMode (const Pin * pin, Pin::Mode m) {
    PioBank * b;
    int i, r, v;
    int g = pin->mcuNumber();
    int f = g;
...
// -----------------------------------------------------------------------------
  Pin::Pull
  DeviceRaspberryPi::pull (const Pin * pin) const {
    PioBank * b;
    int i, r, v;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  void
  DeviceRaspberryPi::setPull (const Pin * pin, Pin::Pull p) {
    PioBank * b;
    int v = -1;
    int g = pin->mcuNumber();
    int f = g;
...
// -----------------------------------------------------------------------------
  void
  DeviceRaspberryPi::write (const Pin * pin, bool v) {
    PioBank * b;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  void
  DeviceRaspberryPi::toggle (const Pin * pin) {
    PioBank * b;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  bool
  DeviceRaspberryPi::read (const Pin * pin) const {
    PioBank * b;
    int g = pin->mcuNumber();
...
// -----------------------------------------------------------------------------
  ...
  DeviceRaspberryPi::modes() const {
    return _modes;
  }


// -----------------------------------------------------------------------------
  const std::map<Pin::Mode, std::string> DeviceRaspberryPi::_modes = {
    {Pin::ModeInput, "in"},
    {Pin::ModeOutput, "out"},
    {Pin::ModeAlt2, "alt2"},
    {Pin::ModeAlt3, "alt3"},
...
// -----------------------------------------------------------------------------
  const std::map<int, Pin::Mode> DeviceRaspberryPi::_int2mode = {
    {0, Pin::ModeInput},
    {1, Pin::ModeOutput},
    {2, Pin::ModeAlt2},
    {3, Pin::ModeAlt3},
...
// -----------------------------------------------------------------------------
  const std::map<Pin::Mode, int> DeviceRaspberryPi::_mode2int = {
    {Pin::ModeInput, 0},
    {Pin::ModeOutput, 1},
    {Pin::ModeAlt2, 2},
    {Pin::ModeAlt3, 3},
  ...
  */
  const int DeviceRaspberryPi::_portSize[] = {
    22, 0, 17, 18, 16, 7, 14, 12, -1
  };

// -----------------------------------------------------------------------------
  PioBank *
  DeviceRaspberryPi::bank (unsigned int bkindex) const {
    PioBank * bk = nullptr;
...
// -----------------------------------------------------------------------------
  PioBank *
  DeviceRaspberryPi::pinBank (int * mcupin) const {
    PioBank * bk = nullptr;
...
// -----------------------------------------------------------------------------
  ...
  void
  DeviceRaspberryPi::debugPrintAllBanks () const {
...
// -----------------------------------------------------------------------------
  void
  DeviceRaspberryPi::debugPrintBank (const PioBank * b) const {
