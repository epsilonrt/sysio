...
char ssdv_enc_init(ssdv_t *s, const char *callsign, uint8_t image_id)
{
...
char *ssdv_dec_callsign(char *callsign, uint32_t code)
{
  char *c, s;
...
char ssdv_dec_feed(ssdv_t *s, uint8_t *packet)
{
  ...
  if(s->packet_id == 0)
  {
    ...
    /* Display information about the image */
    fprintf(stderr, "Callsign: %s\n", ssdv_dec_callsign(callsign, s->callsign));
