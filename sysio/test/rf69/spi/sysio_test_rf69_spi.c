...
main (int argc, char **argv) {
  ...
  ret = iSpiWriteReg8 (fd, addr, 0xB8);
  assert (ret >= 0);
  ret = iSpiReadReg8 (fd, addr);
  assert (ret ==  0xB8);
  ...
  printf ("Test %d>\n", t++);
  addr = 0;
  while ( (strcmp (regs[addr], "RegSyncValue1") != 0) && regs[addr] != NULL) {

    addr++;
  }
  if (regs[addr]) {

    ret = iSpiReadRegBlock (fd, addr, backup, 8);
    assert (ret >= 0);
  ...
  if (regs[addr]) {
    ...
    ret = iSpiWriteRegBlock (fd, addr, tx_buffer, 8);
    assert (ret == 8);
    ret = iSpiReadRegBlock (fd, addr, rx_buffer, 8);
    assert (ret == 8);
    printf ("0x%02X,%s,%d,0x", addr, regs[addr], ret);
    for (int i = 0; i < 8; i++) {
    ...
    putchar ('\n');
    assert (memcmp (tx_buffer, rx_buffer, 8) == 0);
  }
  printf ("Success\n\n");

  ret = iSpiWriteRegBlock (fd, addr, backup, 8);
  assert (ret == 8);
